...
# Prefer the locally built SM120 CUDA 13 torch if present
...
PY
    then
        echo "[speedrun] Local import failed, falling back to nightly wheels"
...
python -m nanochat.torchrun_no_libuv --standalone --nproc_per_node=8 -m scripts.base_train -- --depth=32 --device_batch_size=8
...
python -m nanochat.torchrun_no_libuv --standalone --nproc_per_node=8 -m scripts.mid_train -- --device_batch_size=8 --run=$WANDB_RUN
