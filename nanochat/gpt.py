...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, kv_cache=None, loss_reduction='mean'):
        ...
        x = norm(x)
        use_checkpointing = self.activation_checkpoint and kv_cache is None
        for block in self.transformer.h:
            if use_checkpointing:
                def block_forward(hidden_states, block=block):
                    return block(hidden_states, cos_sin, kv_cache)
